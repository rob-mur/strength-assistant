...
describe("useAppInit", () => {
  ...
  describe("Console Logging", () => {
    it("should log initialization progress", async () => {
      ...
      await waitFor(() => {
        expect(consoleSpy).toHaveBeenCalledWith(
          "🔄 useAppInit: Hook initialized",
        );
        expect(consoleSpy).toHaveBeenCalledWith(
          "🚀 useAppInit: Starting app initialization",
        );
        expect(consoleSpy).toHaveBeenCalledWith(
          "✅ useAppInit: App initialization complete",
        );
      });
    });

    it("should log font loading state", () => {
      const consoleSpy = jest.spyOn(console, "log").mockImplementation();

      renderHook(() => useAppInit());

      expect(consoleSpy).toHaveBeenCalledWith(
        "🔄 useAppInit: Fonts state -",
        expect.objectContaining({
          fontsLoaded: true,
          fontError: false,
        }),
    ...
    it("should log readiness check", async () => {
      const consoleSpy = jest.spyOn(console, "log").mockImplementation();
