...
export const WorkoutSetValidation = z.object({
  weight: z.number()
    ...
    .min(WORKOUT_SET_CONSTRAINTS.weight.min, `Weight must be at least ${WORKOUT_SET_CONSTRAINTS.weight.min}`)
    .max(WORKOUT_SET_CONSTRAINTS.weight.max, `Weight cannot exceed ${WORKOUT_SET_CONSTRAINTS.weight.max}`)
    .transform(val => Math.round(val * 100) / 100), // Round to 2 decimal places for precision
  
  repetitions: z.number()
    .int('Repetitions must be a whole number')
    .min(WORKOUT_SET_CONSTRAINTS.repetitions.min, `At least ${WORKOUT_SET_CONSTRAINTS.repetitions.min} repetition required`)
    .max(WORKOUT_SET_CONSTRAINTS.repetitions.max, `Cannot exceed ${WORKOUT_SET_CONSTRAINTS.repetitions.max} repetitions`),
    
  rpe: z.number()
    .min(WORKOUT_SET_CONSTRAINTS.rpe.min, `RPE must be at least ${WORKOUT_SET_CONSTRAINTS.rpe.min}`)
    .max(WORKOUT_SET_CONSTRAINTS.rpe.max, `RPE cannot exceed ${WORKOUT_SET_CONSTRAINTS.rpe.max}`)
    .refine(
      (val) => {
      ...
      {
        message: 'RPE must be in 0.5 increments (e.g., 1.0, 1.5, 2.0, ..., 9.5, 10.0)'
      }
    ),
 ...
 */
export const UpdateWorkoutSetValidation = WorkoutSetValidation
  .partial()
...
export type UpdateWorkoutSetFormData = z.infer<typeof UpdateWorkoutSetValidation>;
 ...
 */
export function validateWorkoutSet(data: unknown): ValidationResult<WorkoutSetFormData> {
  try {
 ...
 */
export function validateWorkoutSetUpdate(data: unknown): ValidationResult<UpdateWorkoutSetFormData> {
  try {
 ...
 */
export function isValidWorkoutSetData(data: unknown): data is WorkoutSetFormData {
  return WorkoutSetValidation.safeParse(data).success;
}

export function isValidUpdateData(data: unknown): data is UpdateWorkoutSetFormData {
...
export const ValidationPatterns = {
  ...
  positiveDecimal: z.number().positive().transform(val => Math.round(val * 100) / 100),
