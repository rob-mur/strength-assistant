...
describe("StorageManager - Enhanced Functionality", () => {
  ...
  describe("Data Validation and Consistency", () => {
    ...
    it("should detect user email mismatch", async () => {
      ...
      mockSupabaseBackend.getCurrentUser.mockResolvedValue({
        id: "123",
        email: "user1@example.com",
        isAnonymous: false,
      });
      mockFirebaseBackend.getCurrentUser.mockResolvedValue({
        id: "123",
      ...
      expect(result.errors).toContain(
        "User email mismatch: Supabase(user1@example.com) vs Firebase(user2@example.com)",
      );
    ...
    it("should detect user anonymous status mismatch", async () => {
      ...
      expect(result.isConsistent).toBe(false);
      expect(result.errors).toContain(
        "User anonymous status mismatch: Supabase(true) vs Firebase(false)",
      );
    });
    ...
    it("should detect user presence mismatch", async () => {
      ...
      expect(result.isConsistent).toBe(false);
      expect(result.errors).toContain(
        "User presence mismatch: Supabase(true) vs Firebase(false)",
      );
    });
    ...
    it("should detect exercise count mismatch", async () => {
      ...
      expect(result.isConsistent).toBe(false);
      expect(result.errors).toContain(
        "Exercise count mismatch: Supabase(2) vs Firebase(1)",
      );
    });
    ...
    it("should detect missing exercises in Firebase", async () => {
      ...
      expect(result.isConsistent).toBe(false);
      expect(result.errors).toContain(
        'Exercise "Squats" exists in Supabase but not Firebase',
      );
    });
    ...
    it("should detect missing exercises in Supabase", async () => {
      ...
      expect(result.isConsistent).toBe(false);
      expect(result.errors).toContain(
        'Exercise "Deadlifts" exists in Firebase but not Supabase',
      );
    });

    it("should handle validation errors gracefully", async () => {
      // Mock backends to throw errors
      mockSupabaseBackend.getCurrentUser.mockRejectedValue(
        new Error("Supabase connection error"),
      );
      mockFirebaseBackend.getCurrentUser.mockRejectedValue(
  ...
  describe("User Data Migration", () => {
    ...
    it("should successfully migrate user data", async () => {
      ...
      await storageManager.migrateUserData(
        mockSupabaseBackend,
        mockFirebaseBackend,
      );
      ...
      expect(mockFirebaseBackend.createExercise).toHaveBeenCalledWith({
        name: "Push-ups",
        userId: "123",
      });
      expect(mockFirebaseBackend.createExercise).toHaveBeenCalledWith({
      ...
      expect(mockConsole.info).toHaveBeenCalledWith(
        "🔄 Migrating 2 exercises for user test@example.com",
      );
      expect(mockConsole.info).toHaveBeenCalledWith(
    ...
    it("should handle migration with anonymous user", async () => {
      ...
      expect(mockConsole.info).toHaveBeenCalledWith(
        "🔄 Migrating 1 exercises for user anonymous",
      );
    ...
    it("should handle migration errors during exercise creation", async () => {
      ...
      await expect(
        storageManager.migrateUserData(
          mockSupabaseBackend,
          mockFirebaseBackend,
        ),
      ).rejects.toThrow(
        "User data migration failed: Database connection failed",
      );
    ...
    it("should handle migration errors gracefully", async () => {
      ...
      await expect(
        ...
        "User data migration failed: No user found in source backend",
      );
    });
  });
  ...
  describe("Production Safety", () => {
    ...
    it("should allow clearAllData in development", async () => {
      ...
      expect(mockConsole.info).toHaveBeenCalledWith(
        "🗑️ Cleared all data from both backends",
      );
