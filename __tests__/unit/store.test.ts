...
describe("workoutSetStore", () => {
  ...
  describe("workout set actions", () => {
    describe("createSet", () => {
      ...
      it("handles creation errors gracefully", async () => {
        workoutSets.create = jest
          .fn()
          .mockRejectedValue(new Error("Network error"));

        await expect(
          workoutSetActions.createSet(mockSetRequest),
    ...
    describe("deleteSet", () => {
      ...
      it("resets form defaults after deleting last set", async () => {
        ...
        jest
          .spyOn(workoutSetActions, "getSessionSets")
          .mockReturnValue(mockCurrentSessionSets);
    ...
    describe("setCurrentSession", () => {
      it("sets session context and loads existing sets", () => {
        ...
        jest
          .spyOn(workoutSetActions, "getSessionSets")
          .mockReturnValue(mockSessionSets);
  ...
  describe("data consistency", () => {
    it("maintains set order consistency", () => {
      ...
      const sorted = workoutSetActions.getSessionSets(
        "exercise-123",
        "2025-11-19",
      );
      jest
        .spyOn(workoutSets, "get")
        .mockReturnValue(
          sets.reduce((acc, set) => ({ ...acc, [set.id]: set }), {}),
        );

      // The getSessionSets should sort by set_order
      expect(sorted[0]?.set_order).toBeLessThanOrEqual(
        sorted[1]?.set_order || 0,
      );
    });
