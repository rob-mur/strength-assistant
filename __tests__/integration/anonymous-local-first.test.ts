...
describe("Anonymous Local-First Storage Integration", () => {
  ...
  describe("Local Exercise Management", () => {
    ...
    it("should update exercises locally", async () => {
      ...
      expect(new Date(updatedExercises[0].updatedAt).getTime()).toBeGreaterThan(
        new Date(updatedExercises[0].createdAt).getTime(),
      );
    });
  ...
  describe("Data Consistency and Validation", () => {
    ...
    it("should validate consistency across operations", async () => {
      ...
      expect(exercises.find((e) => e.id === exercises[1].id)?.name).toBe(
        "Updated Exercise B",
      );
    ...
    it("should validate data after app restart", async () => {
      ...
      expect(postRestartSyncState.pendingChanges).toBe(
        preRestartSyncState.pendingChanges,
      );

      preRestartExercises.forEach((preExercise) => {
        const postExercise = postRestartExercises.find(
          (e) => e.id === preExercise.id,
        );
  ...
  describe("Performance and Memory", () => {
    ...
    it("should handle memory efficiently for large datasets", async () => {
      ...
      expect(afterCleanupMemory.heapUsed).toBeLessThan(
        afterCreationMemory.heapUsed,
      );
    });
  });
