...
describe('ExerciseRecord', () => {
  ...
  describe('createExerciseRecord', () => {
    ...
    it('should throw error for invalid name', () => {
      ...
      expect(() => createExerciseRecord(input)).toThrow(ExerciseValidationError);
    });
  });
  ...
  describe('updateExerciseRecord', () => {
    ...
    it('should throw error for empty updates', () => {
      expect(() => updateExerciseRecord(existingRecord, {})).toThrow(ExerciseValidationError);
      expect(() => updateExerciseRecord(existingRecord, {})).toThrow('No updates provided');
    });
    ...
    it('should throw error for invalid name', () => {
      ...
      expect(() => updateExerciseRecord(existingRecord, updates)).toThrow(ExerciseValidationError);
    });
  });
  ...
  describe('validateExerciseName', () => {
    ...
    it('should throw error for non-string input', () => {
      expect(() => validateExerciseName(null as any)).toThrow(ExerciseValidationError);
      expect(() => validateExerciseName(undefined as any)).toThrow(ExerciseValidationError);
      expect(() => validateExerciseName(123 as any)).toThrow(ExerciseValidationError);
    });
    ...
    it('should throw error for too long names', () => {
      ...
      expect(() => validateExerciseName(longName)).toThrow(ExerciseValidationError);
    });
    ...
    it('should throw error for names with invalid characters', () => {
      expect(() => validateExerciseName('Exercise<script>')).toThrow(ExerciseValidationError);
      expect(() => validateExerciseName('Exercise"quote')).toThrow(ExerciseValidationError);
      expect(() => validateExerciseName("Exercise'quote")).toThrow(ExerciseValidationError);
      expect(() => validateExerciseName('Exercise&amp')).toThrow(ExerciseValidationError);
      expect(() => validateExerciseName('Exercise>tag')).toThrow(ExerciseValidationError);
    });
  });
  ...
  describe('validateExerciseRecord', () => {
    ...
    it('should throw error for invalid userId', () => {
      validRecord.userId = '';
      expect(() => validateExerciseRecord(validRecord)).toThrow(ExerciseValidationError);
    ...
    it('should throw error for missing or invalid ID', () => {
      validRecord.id = '';
      expect(() => validateExerciseRecord(validRecord)).toThrow(ExerciseValidationError);
      expect(() => validateExerciseRecord(validRecord)).toThrow('Exercise ID is required');

      validRecord.id = 'invalid-uuid';
      expect(() => validateExerciseRecord(validRecord)).toThrow(ExerciseValidationError);
      expect(() => validateExerciseRecord(validRecord)).toThrow('Exercise ID must be a valid UUID');
    });

    it('should throw error for invalid timestamps', () => {
      validRecord.createdAt = new Date('invalid');
      expect(() => validateExerciseRecord(validRecord)).toThrow(ExerciseValidationError);
      expect(() => validateExerciseRecord(validRecord)).toThrow('Invalid createdAt timestamp');

      validRecord.createdAt = new Date('2023-01-01');
      validRecord.updatedAt = new Date('invalid');
      expect(() => validateExerciseRecord(validRecord)).toThrow(ExerciseValidationError);
      expect(() => validateExerciseRecord(validRecord)).toThrow('Invalid updatedAt timestamp');
    });
    ...
    it('should throw error when updatedAt is before createdAt', () => {
      ...
      expect(() => validateExerciseRecord(validRecord)).toThrow(ExerciseValidationError);
      expect(() => validateExerciseRecord(validRecord)).toThrow('updatedAt cannot be before createdAt');
    });

    it('should throw error for invalid sync status', () => {
      (validRecord as any).syncStatus = 'invalid';
      expect(() => validateExerciseRecord(validRecord)).toThrow(ExerciseValidationError);
      expect(() => validateExerciseRecord(validRecord)).toThrow('Invalid sync status');
    });
  });
  ...
  describe('ExerciseSort', () => {
    ...
    it('should sort by created date', () => {
      ...
      expect(sorted.map(e => e.id)).toEqual(['22222222-2222-4222-8222-222222222222', '33333333-3333-4333-8333-333333333333', '11111111-1111-4111-8111-111111111111']);
    });
    ...
    it('should sort by name alphabetically', () => {
      ...
      expect(sorted.map(e => e.name)).toEqual(['Alpha Exercise', 'Beta Exercise', 'Zebra Exercise']);
    });

    it('should sort by sync status (error, pending, synced)', () => {
      const sorted = exercises.sort(ExerciseSort.bySyncStatus);
      expect(sorted.map(e => e.syncStatus)).toEqual(['error', 'pending', 'synced']);
    });
  });
});