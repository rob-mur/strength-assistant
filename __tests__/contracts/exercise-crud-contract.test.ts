...
describe('Exercise CRUD Contract', () => {
  ...
  describe('Local-First Exercise Retrieval', () => {
    ...
    it('should return exercises sorted by creation date', async () => {
      ...
      expect(exercises[0].createdAt.getTime()).toBeLessThanOrEqual(exercises[1].createdAt.getTime());
      expect(exercises[1].createdAt.getTime()).toBeLessThanOrEqual(exercises[2].createdAt.getTime());
    });
  });
  ...
  describe('Local-First Exercise Updates', () => {
    ...
    it('should update exercise locally with immediate response', async () => {
      ...
      expect(updated.updatedAt.getTime()).toBeGreaterThan(testExercise.updatedAt.getTime());
      expect(updated.syncStatus).toBe('pending');
    });

    it('should validate update parameters', async () => {
      // Invalid exercise ID
      await expect(exerciseService.updateExercise('invalid-id', { name: 'New Name' }))
        .rejects.toThrow('Exercise not found');

      // Empty name
      await expect(exerciseService.updateExercise(testExercise.id, { name: '' }))
        .rejects.toThrow('Exercise name cannot be empty');

      // No updates provided
      await expect(exerciseService.updateExercise(testExercise.id, {}))
  ...
  describe('Local-First Exercise Deletion', () => {
    ...
    it('should delete exercise locally with immediate response', async () => {
      ...
      await expect(exerciseService.getExercise(testExercise.id))
        .rejects.toThrow('Exercise not found');
    ...
    it('should handle deletion of non-existent exercise', async () => {
      await expect(exerciseService.deleteExercise('non-existent-id'))
        .rejects.toThrow('Exercise not found');
    });
  ...
  describe('Offline Persistence', () => {
    ...
    it('should maintain sync status across app restarts', async () => {
      ...
      const restoredExercise = await newServiceInstance.getExercise(exercise.id);
