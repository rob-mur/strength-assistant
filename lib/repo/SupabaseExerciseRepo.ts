...
export class SupabaseExerciseRepo implements IExerciseRepo {
  ...
  private async validateUserAuthentication(
    ...
    // Handle empty or invalid userId by falling back to getCurrentUser()
    if (!userId || typeof userId !== "string" || userId.trim() === "") {
      console.log(
        "🔐 SupabaseExerciseRepo - Empty userId provided, falling back to getCurrentUser()",
      );
      // Fall through to Supabase validation to get current user
    } else {
      // For local anonymous users (generated by createAnonymousUser),
      // skip Supabase validation and trust the provided userId
      // Local anonymous users are UUIDs that don't exist in Supabase
      const uuidRegex =
    ...
    // For real Supabase users or empty userId, validate against Supabase
    console.log(
    ...
    try {
      // Add a 1-second timeout for Supabase validation
