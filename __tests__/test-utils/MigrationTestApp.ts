...
export class MigrationTestApp extends TestApp {
  ...
  async addExerciseWithBackendValidation(name: string): Promise<{
    ...
    return {
      exercise,
      backend,
      syncStatus: syncStatus.hasErrors
        ? syncStatus.errorMessage || "error"
        : "success",
    };
  }
  ...
  async restart(): Promise<void> {
    ...
    const envValue =
      process.env.USE_SUPABASE_DATA ||
      process.env.EXPO_PUBLIC_USE_SUPABASE_DATA;
