...
describe("syncConfig functions", () => {
  ...
  beforeEach(() => {
    ...
    (supabaseClient.onAuthStateChange as jest.Mock).mockImplementation(
      (callback) => {
  ...
  describe("configureSyncEngine", () => {
    ...
    test("handles errors during initial data loading", async () => {
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith(
        "Failed to load initial exercises:",
        loadError,
      );
    });
    ...
    test("handles errors during subscription setup", async () => {
      ...
      (supabaseClient.getCurrentUser as jest.Mock).mockRejectedValue(
        subscriptionError,
      );
  ...
  describe("syncExerciseToSupabase", () => {
    ...
    test("throws error when user not authenticated", async () => {
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith(
        "Failed to sync exercise to Supabase:",
        expect.any(Error),
      );
    });
  ...
  describe("deleteExerciseFromSupabase", () => {
    ...
    test("throws error when Supabase delete fails", async () => {
      ...
      await expect(
        deleteExerciseFromSupabase(exerciseId, userId),
      ).rejects.toThrow("Delete failed");

      expect(consoleErrorSpy).toHaveBeenCalledWith(
        "Failed to delete exercise from Supabase:",
        supabaseError,
      );
    });
  });
