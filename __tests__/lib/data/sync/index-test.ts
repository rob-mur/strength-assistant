...
describe('Data Sync Index', () => {
  ...
  describe('initializeDataLayer', () => {
    it('initializes all components successfully', async () => {
      ...
      expect(console.log).toHaveBeenCalledWith('✅ Offline-first data layer initialized successfully');
    });
    ...
    it('initializes components in correct order', async () => {
      ...
      const configureSyncEngineCallOrder = configureSyncEngine.mock.invocationCallOrder[0];
    ...
    it('logs initialization steps', async () => {
      ...
      expect(console.log).toHaveBeenNthCalledWith(1, '🔥 Initializing Firebase...');
      expect(console.log).toHaveBeenNthCalledWith(2, '📊 Initializing Supabase...');
      expect(console.log).toHaveBeenNthCalledWith(3, '🔄 Configuring sync engine...');
      expect(console.log).toHaveBeenNthCalledWith(4, '✅ Offline-first data layer initialized successfully');
    });
  });
  ...
  describe('Error Handling', () => {
    it('handles Firebase initialization errors in non-web environment', async () => {
      ...
      await expect(initializeDataLayer()).rejects.toThrow('Firebase initialization failed');
      expect(console.error).toHaveBeenCalledWith('❌ Failed to initialize data layer:', expect.any(Error));
    });
    ...
    it('handles Supabase initialization errors in non-web environment', async () => {
      ...
      await expect(initializeDataLayer()).rejects.toThrow('Supabase initialization failed');
      expect(console.error).toHaveBeenCalledWith('❌ Failed to initialize data layer:', expect.any(Error));
    });
    ...
    it('handles sync engine configuration errors in non-web environment', async () => {
      ...
      await expect(initializeDataLayer()).rejects.toThrow('Sync engine configuration failed');
      expect(console.error).toHaveBeenCalledWith('❌ Failed to initialize data layer:', expect.any(Error));
    });
    ...
    it('continues with degraded functionality in web environment', async () => {
      ...
      expect(console.error).toHaveBeenCalledWith('❌ Failed to initialize data layer:', expect.any(Error));
      expect(console.warn).toHaveBeenCalledWith('⚠️ Continuing with degraded functionality for web environment');
