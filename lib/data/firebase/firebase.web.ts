  ...
  private setupEmulator(): void {
    ...
    if (this.isEmulatorEnabled()) {
      ...
      try {
        ...
        if (
          !(
            this.db as { _delegate?: { _databaseId?: { database?: string } } }
          )._delegate?._databaseId?.database?.includes("emulator")
        ) {
        ...
        this.logWarn(
          "Continuing without emulator for Chrome testing compatibility",
        );
      }
    } else {
  ...
  getDb(): Firestore {
    ...
    if (!this.db) {
      const error = new Error(
        "Firestore instance not available. This may indicate an initialization timing issue in web environment.",
      );
