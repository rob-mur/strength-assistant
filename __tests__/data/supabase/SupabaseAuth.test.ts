...
describe("SupabaseAuth", () => {
  ...
  describe("signUp", () => {
    ...
    it("should throw error for invalid email format", async () => {
      await expect(auth.signUp("invalid-email", "password123")).rejects.toThrow(
        "Invalid email format",
      );
    });

    it("should throw error for short password", async () => {
      await expect(auth.signUp("test@example.com", "123")).rejects.toThrow(
  ...
  describe("signInAnonymously", () => {
    ...
    it("should throw error when Supabase returns error", async () => {
      ...
      await expect(auth.signInAnonymously()).rejects.toThrow(
        "Anonymous sign in failed: Anonymous auth disabled",
      );
    });
  ...
  describe("subscribeToAuthState", () => {
    ...
    it("should call callbacks when auth state changes", () => {
      ...
      const callIndex =
        mockSupabaseClient.auth.onAuthStateChange.mock.calls.length - 1;
      const authStateChangeCallback =
        mockSupabaseClient.auth.onAuthStateChange.mock.calls[callIndex][0];
  ...
  describe("linkEmailPassword", () => {
    it("should link email/password to anonymous user successfully", async () => {
      ...
      const result = await auth.linkEmailPassword(
        "test@example.com",
        "password123",
      );
    ...
    it("should throw error for missing email", async () => {
      await expect(auth.linkEmailPassword("", "password123")).rejects.toThrow(
    ...
    it("should throw error for missing password", async () => {
      await expect(
        auth.linkEmailPassword("test@example.com", ""),
      ).rejects.toThrow("Email and password are required");
    });

    it("should throw error for invalid email format", async () => {
      await expect(
        auth.linkEmailPassword("invalid-email", "password123"),
      ).rejects.toThrow("Invalid email format");
    });

    it("should throw error for weak password", async () => {
      await expect(
        auth.linkEmailPassword("test@example.com", "123"),
    ...
    it("should throw error when no anonymous user exists", async () => {
      ...
      await expect(
        auth.linkEmailPassword("test@example.com", "password123"),
  ...
  describe("forceSessionExpiry", () => {
    it("should force session expiry", async () => {
      mockSupabaseClient.auth.forceSessionExpiry.mockResolvedValueOnce(
        undefined,
      );
  ...
  describe("Email and Password Validation", () => {
    describe("isValidEmail", () => {
      ...
      it("should reject invalid email formats", async () => {
        await expect(
          auth.signUp("invalid.email", "password123"),
        ).rejects.toThrow("Invalid email format");
        await expect(auth.signUp("@domain.com", "password123")).rejects.toThrow(
          "Invalid email format",
        );
        await expect(auth.signUp("user@", "password123")).rejects.toThrow(
          "Invalid email format",
        );
        await expect(auth.signUp("user@domain", "password123")).rejects.toThrow(
          "Invalid email format",
        );
      });
    });
    ...
    describe("isValidPassword", () => {
      it("should validate password length", async () => {
        await expect(auth.signUp("test@example.com", "12345")).rejects.toThrow(
          "Password must be at least 6 characters long",
        );
