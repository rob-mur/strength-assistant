...
describe("Feature Flag Migration Flow", () => {
  ...
  describe("Firebase to Supabase Migration", () => {
    it("should maintain app behavior when switching from Firebase to Supabase", async () => {
      ...
      expect(firebaseExercises.map((e: any) => e.name)).toContain(
        "Bench Press",
      );
      ...
      expect(supabaseExercises.map((e: any) => e.name)).toContain(
        "Bench Press",
      );
      ...
      expect(afterUpdate.find((e: any) => e.id === afterAdd[0].id)?.name).toBe(
        "Updated Exercise",
      );
  ...
  describe("Development and Testing Support", () => {
    it("should provide debug information for migration state", async () => {
      ...
      expect(debugInfo).toMatchObject({
        ...
        featureFlag: false,
        migrationPhase: expect.stringMatching(
          /firebase|supabase|dual-write|cleanup/,
        ),
    ...
    it("should support migration testing in development mode", async () => {
      ...
      expect(logs.some((log: any) => log.includes("Migration test mode"))).toBe(
        true,
      );
    });
  });
