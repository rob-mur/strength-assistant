...
export class ReactNativeErrorHandlerImpl implements ReactNativeErrorHandler {
  ...
  handleUncaughtError(error: Error, isFatal: boolean): void {
    try {
      ...
      this.errorLogger.logAndBlock(error, isFatal ? 'react-native-fatal' : 'react-native-non-fatal');
      ...
      console.error('[ReactNativeErrorHandler] Error while handling error:', handlingError);
  ...
  setupGlobalErrorHandling(): void {
    try {
      ...
      console.error('[ReactNativeErrorHandler] Failed to setup global error handling:', setupError);
    }
  }
  ...
  cleanup(): void {
    try {
      ...
      console.error('[ReactNativeErrorHandler] Failed to cleanup error handlers:', cleanupError);
    }
  }
}
