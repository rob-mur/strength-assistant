...
import type { MockFactoryCollection } from "../../../specs/001-we-are-actually/contracts/test-infrastructure";
...
export class MockFactoryRegistry {
  ...
  createExercise(overrides?: Partial<Exercise>): Exercise {
    ...
    try {
      const exercise =
        this._collection.exerciseFactory.createExercise(overrides);
      ...
      if (this._mockValidationEnabled) {
        ...
        if (!validation.isValid) {
          ...
          console.warn(
            `Exercise mock validation failed: ${validation.errors.join(", ")}`,
          );
        }
      }
    ...
    try {
      ...
      if (this._mockValidationEnabled) {
        ...
        if (!validation.isValid) {
          this._statistics.validationFailures++;
          console.warn(
            `User mock validation failed: ${validation.errors.join(", ")}`,
          );
        }
      }
  ...
  createAuthenticatedUser(
    ...
    try {
      const user = this._collection.userFactory.createAuthenticatedUser(
        email,
        overrides,
      );
      const normalizedUser = this._normalizeUserForBackend(user);
      ...
      if (this._mockValidationEnabled) {
        ...
        if (!validation.isValid) {
          this._statistics.validationFailures++;
          console.warn(
            `Authenticated user mock validation failed: ${validation.errors.join(", ")}`,
          );
        }
      }
  ...
  private _validateExerciseMock(exercise: Exercise): MockValidationResult {
    ...
    if (typeof exercise.deleted !== "boolean")
      errors.push("Exercise deleted must be boolean");
  ...
  private _validateUserMock(user: UserAccount): MockValidationResult {
    ...
    if (typeof user.isAnonymous !== "boolean")
      errors.push("User isAnonymous must be boolean");
    ...
    if (
      this._config.backendType === "firebase" &&
      !/^[a-zA-Z0-9]{28}$/.exec(user.id)
    ) {
      warnings.push("Firebase user ID typically follows 28-character format");
    }
    if (
      this._config.backendType === "supabase" &&
      !/^[a-f0-9-]{36}$/.exec(user.id)
    ) {
  ...
  private _normalizeUserForBackend(user: UserAccount): UserAccount {
    ...
    if (
      ...
      !this._isValidISODate(normalized.createdAt.toString())
    ) {
  ...
  private _updateStatistics(mockType: string, creationTime: number): void {
    ...
    this._statistics.mocksByType[mockType] =
      (this._statistics.mocksByType[mockType] || 0) + 1;
