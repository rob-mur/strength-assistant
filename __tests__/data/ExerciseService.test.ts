...
import {
  ...
  SyncRecord,
  SyncStatus,
  ExerciseWithSyncStatus,
} from "../../lib/data/ExerciseService";
...
describe("ExerciseService - Enhanced Coverage", () => {
  ...
  describe("Error Conditions - getExerciseById", () => {
    it("should throw error when exercise not found", async () => {
      await expect(
        exerciseService.getExerciseById("nonexistent"),
    ...
    it("should throw error when accessing exercise for different user", async () => {
      ...
      await expect(
        exerciseService.getExerciseById(exercise.id, "different-user"),
      ).rejects.toThrow("Exercise not found");
    });
  ...
  describe("Error Conditions - updateExercise", () => {
    ...
    it("should throw error for missing exercise ID", async () => {
      await expect(
        exerciseService.updateExercise("", { name: "Updated" }),
      ).rejects.toThrow("Exercise ID is required");
    });

    it("should throw error when exercise not found", async () => {
      await expect(
        exerciseService.updateExercise("nonexistent", { name: "Updated" }),
      ).rejects.toThrow("Exercise not found");
    });
    ...
    it("should throw error when updating deleted exercise", async () => {
      ...
      await expect(
        exerciseService.updateExercise(exercise.id, { name: "Updated" }),
    ...
    it("should throw error for empty exercise name update", async () => {
      await expect(
        exerciseService.updateExercise(exercise.id, { name: "" }),
    ...
    it("should throw error for exercise name too long", async () => {
      const longName = "a".repeat(101);
      await expect(
        exerciseService.updateExercise(exercise.id, { name: longName }),
      ).rejects.toThrow("Exercise name too long");
    });
  });
  ...
  describe("Error Conditions - deleteExercise", () => {
    ...
    it("should throw error for missing exercise ID", async () => {
      await expect(exerciseService.deleteExercise("")).rejects.toThrow(
    ...
    it("should throw error when exercise not found", async () => {
      await expect(
        exerciseService.deleteExercise("nonexistent"),
      ).rejects.toThrow("Exercise not found");
    });

    it("should throw error when deleting exercise for different user", async () => {
      await expect(
        exerciseService.deleteExercise(exercise.id, "different-user"),
      ).rejects.toThrow(
  ...
  describe("Sync Management", () => {
    ...
    it("should get sync record by exercise ID", async () => {
      ...
      const syncRecord = await exerciseService.getSyncRecordByExerciseId(
        exercise.id,
      );
    ...
    it("should return undefined for non-existent sync record", async () => {
      const syncRecord =
        await exerciseService.getSyncRecordByExerciseId("nonexistent");
    ...
    it("should update sync record status", async () => {
      ...
      if (syncRecord) {
        await exerciseService.updateSyncRecordStatus(
          syncRecord.id,
          "error",
          "Test error",
        );

        const updated = await exerciseService.getSyncRecordByExerciseId(
          exercise.id,
        );
  ...
  describe("User Management", () => {
    ...
    it("should set current user", async () => {
      await exerciseService.setCurrentUser({
        id: "test-user",
        isAnonymous: false,
      });
