...
jest.mock('react-native-paper', () => {
  ...
  const Card = ({ children, style }: any) => <View testID="profile-card" style={style}>{children}</View>;
  Card.Content = ({ children }: any) => <View testID="card-content">{children}</View>;
  
  const List = {
    Item: ({ title, description, left, testID }: any) => (
      <View testID={testID || `list-item-${title.toLowerCase().replace(' ', '-')}`}>
    ...
    Icon: ({ icon, color, style, testID }: any) => (
      <View testID={testID || `icon-${icon}`} style={style} data-color={color} data-icon={icon} />
    ),
  };
  ...
  return {
    ...
    Text: ({ children, variant, style, testID, ...props }: any) => (
      <Text testID={testID || `text-${variant}`} style={style} {...props}>{children}</Text>
    ),
    Button: ({ children, onPress, mode, style, buttonColor, testID, ...props }: any) => (
      <TouchableOpacity 
        ...
        {...props}
      >
        <Text testID="button-text" data-mode={mode} data-button-color={buttonColor}>{children}</Text>
      </TouchableOpacity>
    ),
    List,
    Divider: ({ style, testID }: any) => <View testID={testID || 'divider'} style={style} />,
  };
});
...
describe('ProfileScreen', () => {
  ...
  describe('Component Rendering with Registered User', () => {
    ...
    it('should display user ID correctly', () => {
      ...
      const titleElement = userIdItem.findByProps({ testID: 'list-item-title' });
      const descriptionElement = userIdItem.findByProps({ testID: 'list-item-description' });
    ...
    it('should display email correctly for registered user', () => {
      ...
      const descriptionElement = emailItem.findByProps({ testID: 'list-item-description' });
    ...
    it('should display registered account type', () => {
      ...
      const titleElement = accountTypeItem.findByProps({ testID: 'list-item-title' });
      const descriptionElement = accountTypeItem.findByProps({ testID: 'list-item-description' });
    ...
    it('should display registered user sign out description', () => {
      ...
      expect(description.props.children).toBe('Sign out of your account. You can sign back in anytime.');
    });
  });
  ...
  describe('Component Rendering with Anonymous User', () => {
    ...
    it('should display anonymous user sign out description', () => {
      ...
      const description = getByTestId('text-bodyMedium');
      expect(description.props.children).toBe('Sign out of your guest account. Your data will be lost unless you create a permanent account first.');
    });
  });
  ...
  describe('Icon Components', () => {
    ...
    it('should render account-check icon for registered user', () => {
      ...
      const accountTypeItem = getByTestId('list-item-account-type');
      const icon = accountTypeItem.findByProps({ testID: 'icon-account-check' });
  ...
  describe('Edge Cases', () => {
    it('should handle undefined user properties', () => {
      ...
      const userIdItem = getByTestId('list-item-user-id');
      const userIdDescription = userIdItem.findByProps({ testID: 'list-item-description' });
