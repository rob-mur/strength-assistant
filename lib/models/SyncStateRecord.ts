...
export function recordSyncFailure(
  ...
  const nextRetryDelay = SYNC_CONFIG.RETRY_DELAYS[syncState.attempts] || 
...
export function validateSyncStateRecord(syncState: SyncStateRecord): void {
  ...
  if (!(syncState.pendingSince instanceof Date) || isNaN(syncState.pendingSince.getTime())) {
    throw new SyncValidationError('Invalid pendingSince timestamp', 'pendingSince');
  }
  ...
  if (syncState.nextRetryAt && (!(syncState.nextRetryAt instanceof Date) || isNaN(syncState.nextRetryAt.getTime()))) {
    throw new SyncValidationError('Invalid nextRetryAt timestamp', 'nextRetryAt');
  }

  if (syncState.lastError !== undefined && typeof syncState.lastError !== 'string') {
...
export const SyncStateUtils = {
   ...
   */
  groupByRecordType: (syncStates: SyncStateRecord[]): Record<string, SyncStateRecord[]> => {
    return syncStates.reduce((groups, state) => {
   ...
   */
  groupByOperation: (syncStates: SyncStateRecord[]): Record<string, SyncStateRecord[]> => {
    return syncStates.reduce((groups, state) => {
  ...
  getNextRetryTime: (syncStates: SyncStateRecord[]): Date | null => {
    const waitingStates = syncStates
      .filter(state => !hasFailedPermanently(state) && !isReadyForRetry(state))
